...
import { getTotal } from "src/features/CartSlice"
...
export function Navigation() {
  ...
  const cartTotal = useSelector((state:RootState) => state.cartProducts.cartTotal)
  ...
  <div className="relative max-w-[1280px] m-auto flex justify-end p-8 gap-8 items-center ">
      ...
      <div className=" flex flex-grow">
        ...
        <h1 className="text-2xl font-semibold text-red-400"><em>Polished</em></h1>
      </div>
      ...
      <div className="relative flex items-center gap-2 cursor-pointer" onClick={togleShowCart}>
              <span className="absolute z-10 left-3 -top-1 border border-white bg-red-400 rounded-[50%] text-[0.5rem] text-white px-1">{cartProducts.length}</span>
      ...
      {showCart && <div className="w-full sm:w-[500px] absolute z-50 shadow-lg top-[96px] right-0 sm:right-10 flex flex-col bg-white border p-6  ">
